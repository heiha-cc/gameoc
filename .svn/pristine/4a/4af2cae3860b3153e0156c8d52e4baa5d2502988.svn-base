...
//UINT32		aiRoleID[1];			//用户ID数组
    function SendMDQueryRoleList(&$socket, $iRoleCount, $aiRoleID)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_ROLE_LIST, $in_stream->len, 0, REQ_OM, REQ_DC);
...
//UINT32		iRoleID;				//角色ID
    function SendMDGetRoleBaseInfo(&$socket, $iRoleID)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_GET_ROLE_BASE_INFO, $in_stream->len, 0, REQ_OM, REQ_DC);
        $in_head_len = COMM_HEAD_LEN;
...
//UINT32		iPageSize;				//每页数量
    function SendMDQueryRoleGameInfo($socket, $iRoleID, $iCurPage, $iPageSize)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_ROLE_GAME_INFO, $in_stream->len, 0, REQ_OM, REQ_DC);
        //$in_head_len = COMM_HEAD_LEN;
...
//char		szNewPwd[33];			//新的银行密码
    function SendMDResetRoleBankPwd($socket, $iRoleID, $szNewPwd)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_RESET_ROLE_BANK_PWD, $in_stream->len, 0, REQ_OM, REQ_DC);
        //$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iMonery;				//金币数量
    function SendMDAddRoleMonery($socket, $iRoleID, $iMonery)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_ADD_ROLE_MONERY, $in_stream->len, 0, REQ_OM, REQ_DC);
//	$in_head_len = COMM_HEAD_LEN;
//
//	$in_len = $in_head_len + $in_stream->len;
//	$in = $in_head . $in_stream->data;
...
//UINT32		iScore;					//积分数量
    function SendMDAddRoleScore($socket, $iRoleID, $iKindID, $iStatus, $iScore)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_ADD_ROLE_SCORE, $in_stream->len, 0, REQ_OM, REQ_DC);
//	$in_head_len = COMM_HEAD_LEN;
//
//	$in_len = $in_head_len + $in_stream->len;
//	$in = $in_head . $in_stream->data;
...
//UINT32		iDays;					//会员天数
    function SendMDBuyRoleVip($socket, $iRoleID, $iDays)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_BUY_ROLE_VIP, $in_stream->len, 0, REQ_OM, REQ_DC);
//	$in_head_len = COMM_HEAD_LEN;
//
//	$in_len = $in_head_len + $in_stream->len;
//	$in = $in_head . $in_stream->data;
...
//	}
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_SAVING_ROLE_MONERY, $in_stream->len, 0, REQ_OM, REQ_DC);
        //$in_head_len = COMM_HEAD_LEN;
...
//在线人数查询接口
    function SendMDQueryOnlinePlayer($socket)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_ONLINE_PLAYER, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iSuperLevel;			//超级会员等级，转账反馈比例 万分之
    function SendMDSetSuperPlayer(&$socket, $iRoleID, $iSuperLevel)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_SET_SUPER_PLAYER, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iLoadType;				//加载类型 0全部
    function SendMDReloadGameData($socket, $iLoadType)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_RELOAD_GAME_DATA, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iRoomID;				//房间ID
    function SendMDActiveRoomRobot($socket, $iRoomID)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_ACTIVE_ROOM_ROBOT, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iPageSize;				//每页数量
    function SendMDQueryRoomRobotInfo($socket, $iCurPage, $iPageSize)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_ROOM_ROBOT_INFO, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iRoleID;				//角色ID
    function SendMDClearCurRoomInfo($socket, $iRoleID)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_CLEAR_CUR_ROOM_INFO, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iRoleID;				//角色ID
    function SendMDQueryRoleTotalMoney($socket, $iRoleID)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_ROLE_TOTAL_MONEY, $in_stream->len, 0, REQ_OM, REQ_DC);
        /* $in_head_len = COMM_HEAD_LEN;
...
//INT64		iDealMoney;				//转账金币数量
    function SendMDSysBankDeal($socket, $iBankAccID, $iToBankAccID, $iDealMoney)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_SYS_BANK_DEAL, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*     $in_head_len = COMM_HEAD_LEN;
...
//INT64		iBankMoney;				//金币数量
    function SendMDAddSysBankMoney($socket, $iBankAccID, $iBankMoney)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_ADD_SYS_BANK_MONEY, $in_stream->len, 0, REQ_OM, REQ_DC);
        /* $in_head_len = COMM_HEAD_LEN;
...
//查询玩家权限
    function SendMDQueryRoleRight($socket, $iRoleID)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_ROLE_RIGHT, $in_stream->len, 0, REQ_OM, REQ_DC);
        //$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iMasterRight;			//管理权限
    function SendMDSetRoleRight($socket, $iRoleID, $iUserRight, $iMasterRight, $iSystemRight)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_SET_ROLE_RIGHT, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*  $in_head_len = COMM_HEAD_LEN;
...
//char		szRoleName[32];			//角色昵称
    function SendMDQueryRoleID($socket, $szRoleName)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_ROLE_ID, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iLuckyEggTax;				//彩蛋抽税
    function SendMDSetLuckyEggTax($socket, $iRoomID, $iLuckyEggTax)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_SET_LUCKY_EGG_TAX, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
...
//查询房间在线玩家
    ...
    function SendMDQueryRoomOnlinePlayers($socket, $iRoomID, $iCurpage, $iPagenum)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_ROOM_ONLINE_PLAYERS, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
    ...
    function SendMDQueryOnlinePlayers($socket)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_ALL_ONLINE_PLAYER, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*$in_head_len = COMM_HEAD_LEN;
...
//UINT32		iCurTime;				//当前时间
    function SendMDQuerySystemBankData($socket, $iCurTime)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_SYSTEM_BANK_DATA, $in_stream->len, 0, REQ_OM, REQ_DC);
        /* $in_head_len = COMM_HEAD_LEN;
...
//UINT32		iCheck;					//是否验证 1验证 0不验证
    function SendMDSetBankWeChatCheck($socket, $iRoleID, $iCheck)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_SET_BANK_WECHAT_CHECK, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*     $in_head_len = COMM_HEAD_LEN;
...
//UINT32		iRoleID;				//角色ID
    function SendMDUnLockUserBank($socket, $iRoleID)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_UNLOCK_USER_BANK, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*     $in_head_len = COMM_HEAD_LEN;
...
//UINT32		iDays;					//会员天数
    function SendMDLockRoleVip($socket, $iRoleID, $iDays)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_LOCK_ROLE_VIP, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*     $in_head_len = COMM_HEAD_LEN;
...
//UINT32		iRoleID;				//角色ID
    function SendMDClearRoleData($socket, $iRoleID)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_CLEAR_ROLE_DATA, $in_stream->len, 0, REQ_OM, REQ_DC);
        /*     $in_head_len = COMM_HEAD_LEN;
    ...
    function SendMDQureySuperUserList($socket, $condition)
    {
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_QUERY_SUPER_USER_LIST, $in_stream->len, 0, REQ_OM, REQ_DC);
        $socket->request($in_head,$in_stream->data);
    }
...
//
    function SendMDCtrolPerson($socket,$AccountId,$Ratio,$ControlTimeLong,$ControlTimeInterval){
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_WD_SET_USER_CTRL_DATA, $in_stream->len, 0, REQ_OM, REQ_DC);
        $socket->request($in_head,$in_stream->data);
...
//发送个人控制
    function SendMDCtrolRoom($socket,$RoomId,$Ratio,$InitStorage,$CurrentStorage,$szStorageRatio){
        ...
        $in_head =  $this->comm->MakeSendHead(CMD_MD_SET_ROOM_CTRL_DATA, $in_stream->len, 0, REQ_OM, REQ_DC);
        $socket->request($in_head,$in_stream->data);
    }
    ...
    function SendMDQueryRoom($socket,$RoomId){
        ...
        $in_head = $this->comm->MakeSendHead(CMD_MD_QUERY_ROOM_CTRL_DATA, $in_stream->len, 0, REQ_OM, REQ_DC);
        $socket->request($in_head,$in_stream->data);
    }
